...
```{r}
test = data %>% filter(typ == "ADSL") %>% select(r_po)
shapiro.test(test$r_po)$p.value

test = data %>% filter(typ == "SATELIT") %>% select(r_po)
shapiro.test(test$r_po)$p.value
...
int1 = data %>% filter(typ == "A1") %>% select(automat)
t.test(int1, alternative="greater", conf.level=0.95)

int2 = data %>% filter(typ == "A2") %>% select(automat)
t.test(int2, alternative="greater", conf.level=0.95)
