::: {#b6219073 .cell .markdown}
\# Cvičení 6. Preprocesing dat a explorační analýza \## Michal Béreš,
Adéla Vrtková, Martina Litschmannová \-\--
:::

::: {#540ecd41 .cell .markdown}
\# 1. Rozšiřující balíčky funkcí - instalace a načítání
:::

::: {#740ac111 .cell .code vscode="{\"languageId\":\"r\"}"}
```{r}
...
::: {#08d77491 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#a3cccb67 .cell .markdown}
Právě kvůli maskování (přepisování existujících funkcí) je dobré omezit
kompletní načtení balíčků na minimum.

Často potřebujeme pouze jednu funkci z balíčku, pak lze tato funkce
načíst pomocí `::`.

Ukážeme si později.
:::

::: {#9faf3130 .cell .markdown}

------------------------------------------------------------------------
:::

::: {#9e79b7bf .cell .markdown}
# 2. Pracovní adresář (working directory) - odkud načítáme a kam ukládáme data {#2-pracovní-adresář-working-directory---odkud-načítáme-a-kam-ukládáme-data}

-   Pozor aktuální otevřená složka v Rstudiu, případně umístění Rskriptu
    není automaticky pracovní adresář!
:::

::: {#1882d279 .cell .code vscode="{\"languageId\":\"r\"}"}
``` R
...
::: {#60bd7580 .cell .code vscode="{\"languageId\":\"r\"}"}
``` R
...
::: {#b318736c .cell .code vscode="{\"languageId\":\"r\"}"}
``` R
getwd() # kde jsme teď
```
:::

::: {#e37586e9 .cell .code vscode="{\"languageId\":\"r\"}"}
``` R
setwd("./..") # zase zpátky
```
:::

::: {#aaef208c .cell .code vscode="{\"languageId\":\"r\"}"}
``` R
getwd() # kontrola
```
:::

::: {#1f297ba5 .cell .markdown}

------------------------------------------------------------------------
:::

::: {#8a9e8127 .cell .markdown}
# 3. Načítání datových souborů {#3-načítání-datových-souborů}

-   pro ověření správnosti načtení budeme použífat funkci `head()` která
    vypíše prvních 6 řádků datového souboru
:::

::: {#78e56a5e .cell .markdown}
### a) Ze souboru CSV

Základní funkce - `read.table`, `read.csv`, `read.csv2`, \...

Záleží hlavně na formátu souboru (.txt, .csv), na tzv. oddělovači
jednotlivých hodnot, desetinné čárce/tečce, \...
:::

::: {#8ea93e05 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#f47d209e .cell .code scrolled="true" vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#d3a7a14d .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#c0dcf13d .cell .markdown}
### b) z Excelu (souboru xlsx)

K načtení/uložení datového souboru ve formátu xlsx použijeme funkci z
balíčku `readxl`

-   konkrétně funkci `readxl::read_excel()`
:::

::: {#328c6686 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#47f3946b .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#84458612 .cell .markdown}
**Poznámka:**

V R jsou aktuálně dva formáty reprezentace tabulek: `data.frame` a
`tibble`.

Objekt importovaný touto cestou bude v novém RStudiu jako typ
\"tibble\".

Jedná se o modernější \"data.frame\" a v některých funkcích může dělat
problémy a házet errory! Jednoduše lze tento objekt převést na typ
data.frame pomocí **as.data.frame()**
:::

::: {#1dad37c8 .cell .markdown}

------------------------------------------------------------------------
:::

::: {#06e5787b .cell .markdown}
# 4. Standardní datový formát (a převod do něj) {#4-standardní-datový-formát-a-převod-do-něj}

Standardní datový formát (v **R** tzv. *\"long format\"*) se vyznačuje
tím, že:

-   **Každý řádek tabulky obsahuje jednu pozorovanou entitu**,
    například:
    ...
    To znamená, že data o produktech dvou firem **nemohou** být v jednom
    řádku -- místo toho se přidá samostatný sloupec s identifikátorem
    firmy.

-   **V jednom sloupci jsou vždy data jednoho typu**:

    -   Je vhodné mít **jeden samostatný sloupec pro `id` entity**
        (například pro odstranění odlehlých pozorování). Například
        pomocí: `data$id <- seq_along(data$...)` nebo s pomocí balíčku
        `dplyr` a funkce `mutate()`:
        `data <- data %>% mutate(id = row_number())`
:::

::: {#fb86899f .cell .markdown}
### a) Základní převod jednoduché datové matice do standardního datového formátu - `stack(...)` {#a-základní-převod-jednoduché-datové-matice-do-standardního-datového-formátu---stack}
:::

::: {#b02f26ed .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#923787a7 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#a40dd7d6 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#5ba2771d .cell .markdown}
### b) Z dat ve formátu Datová matice `reshape(...)` {#b-z-dat-ve-formátu-datová-matice-reshape}
:::

::: {#cf5a56c6 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#e0cf327d .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
-   **`times`** -- Názvy jednotlivých kategorií:
    -   ⚠ **Musí být ve stejném pořadí jako odpovídající sloupce ve
        `varying`!**
-   **`timevar`** -- Název sloupce obsahujícího kategorie.
:::

::: {#c610e528 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
dataS <- reshape(
...
::: {#ea4f9743 .cell .markdown}
### c) Z datového souboru, kde jsou kategorie (např. výrobci) v jednotlivých listech Excelu {#c-z-datového-souboru-kde-jsou-kategorie-např-výrobci-v-jednotlivých-listech-excelu}

V tomto případě bohužel **nelze využít vestavěnou funkci**, která by
provedla operaci automaticky. Musíme to provést ručně následujícím
postupem:
...
3.  **Spojíme všechny proměnné do jedné tabulky** pomocí funkce:
    `r    rbind(data1, data2, data3, ...)`
:::

::: {#633a22dd .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#dd846b6d .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#1b001c67 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#95c155a8 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
# přejmenujeme sloupce
...
::: {#b0e566ee .cell .markdown}

------------------------------------------------------------------------
:::

::: {#23e7b75a .cell .markdown}
# 5. Knihovna Dplyr {#5-knihovna-dplyr}
:::

::: {#af94d1b8 .cell .markdown}
...
-   **`mutate(novy_sloupec = ...)`** vytvoří v datovém rámci nový
    sloupec pomocí výpočtu nad existujícími sloupci:

    ``` r
    data %>% mutate(C = A - B)
    ```

    Tento příkaz vytvoří v datovém rámci `data` nový sloupec `C` jako
    rozdíl hodnot ve stávajících sloupcích `A` a `B`.

-   **`filter(...)`** vyfiltruje záznamy splňující zadané podmínky:

    -   `data %>% filter(vyrobce == "A" | vyrobce == "B")`\
        Vrátí datový rámec, kde sloupec `vyrobce` obsahuje pouze hodnoty
        `"A"` nebo `"B"`.
    -   `data %>% filter(vyrobce == "A", hodnoty > 1000)`\
        Pokud jsou podmínky odděleny čárkou, chápeme je jako **\"a
        zároveň\"**.

-   **`summarise(...)`** vypočítá číselné charakteristiky v rámci
    zadaných sloupců (vhodné pro kombinaci s `group_by`):
...
-   **`group_by(...)`** seskupí data podle unikátních hodnot v zadaném
    sloupci:
...
::: {#f8a15af5 .cell .markdown}
### b) Výběry sloupců
:::

::: {#45ae19d1 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#7dbfc90d .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#139ab551 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#515ff1aa .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#ef58ab02 .cell .markdown}
### c) Vylučování dat ze souboru (pomocí záporných indexů)
:::

::: {#6ec027d4 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
# nebo pomocí dplyr
pokus <- data %>% select(-1, -5)
head(pokus)
...
::: {#ddf0b80a .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
# nebo pomocí názvů
pokus <- data %>% select(-A5, -A22)
head(pokus)
...
::: {#5bae3e3f .cell .markdown}
### d) Práce s daty ve standardním datovém formátu
:::

::: {#0bba0046 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#8b1ebaad .cell .markdown}
#### Definování nových sloupců (`mutate`)
:::

::: {#2540cd4d .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#9b918e4a .cell .markdown}
#### Vybírání řádků a sloupců (`filter` a `select`)
:::

::: {#6321e04b .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#1a3c98b1 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#6db27d7c .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#58b20045 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#a2180031 .cell .markdown}
#### seskupí hodnot do skupin podle zvolené proměnné (`group_by`)

-   Výsledkem je tzv. `grouped_df` (tibble), který obsahuje informace o
    seskupení.
-   Jedná se zejména o mezioperace, které nám umožní provádět další
    operace nad seskupenými daty.
:::

::: {#01e8a320 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#e82db7bf .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#2ec63027 .cell .markdown}
#### setřízení dat (`arrange`)
:::

::: {#f3b94668 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#eb0669dd .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
# případně sestupně
...
::: {#c8dac59b .cell .markdown}

------------------------------------------------------------------------
:::

::: {#eb48f82c .cell .markdown}
# 6. Explorační analýza a vizualizace kategoriální proměnné {#6-explorační-analýza-a-vizualizace-kategoriální-proměnné}
:::

::: {#cd6c7d9e .cell .markdown}
### a) Absolutní a relativní četnosti kategoriální proměnné

-   **`table()`** -- vytvoří tabulku absolutních četností kategoriální
    proměnné ze sloupce datového rámce podle unikátních hodnot.
-   lze i pomocí knihovny `dplyr`:
    -   `r     data %>% count(vyrobce)` nebo
    -   `r     data %>% group_by(vyrobce) %>% summarise(n = n())`
:::

::: {#5f47eb9f .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
abs_cetnosti = as.data.frame(dataS %>% group_by(vyrobce) %>% summarise(cetnost = n()))
abs_cetnosti
```
:::

::: {#a3624bdf .cell .markdown}
**Tabulka relativních četností pomocí `prop.table()`**
:::

::: {#04b53b44 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
rel_cetnosti <- prop.table(cetnosti)*100
rel_cetnosti # výpis
```
:::

::: {#8cbb7651 .cell .markdown}
U tabulek relativních četností je potřeba pohlídat zaokrouhlení a s ním
spojené riziko zaokrouhlovací chyby.
:::

::: {#a165daba .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
rel_cetnosti <- round(rel_cetnosti, digits = 3) # zaokrouhlení na 3 desetinná místa
rel_cetnosti[4] <- 100 - sum(rel_cetnosti[1:3]) # ohlídání zaokrouhlovací chyby
rel_cetnosti
```
:::

::: {#de752a10 .cell .markdown}
### b) Vizualizace pomocí sloupcového grafu
:::

::: {#e1ceaf10 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
# Sloupcový graf
barplot(cetnosti)
```
...
::: {#f15c1a51 .cell .markdown}
**Graf lze uložit v podobě obrázku následovně:**
:::

::: {#93dd08f0 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
# uložení grafu
...
::: {#73f2ea8d .cell .markdown}
**A v podobě PDF:**
:::

::: {#c6b7fcbe .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#c1f3a111 .cell .markdown}

------------------------------------------------------------------------
:::

::: {#34c0cc8e .cell .markdown}
# 7. Explorační analýza a vizualizace kvantitativní proměnné {#7-explorační-analýza-a-vizualizace-kvantitativní-proměnné}
:::

::: {#46741dd8 .cell .markdown}
### a) Základní deskriptivní statistiky kvantitativní proměnné

-   **průměr** pomocí `mean()` (pozor na chybějící hodnoty, dají se
    ignorovat pomocí `na.rm = TRUE`)
-   **medián** pomocí `median()`
-   **rozptyl** pomocí `var()`
-   **směrodatná odchylka** pomocí `sd()`
-   **kvantily** pomocí `quantile()` (např.
    `quantile(data$kap5, c(0.25, 0.75))`)
...
-   **špičatost** pomocí `moments::kurtosis() - 3` (Pozor na
    normalizaci!)

**pozor na chybějící hodnoty, dají se ignorovat pomocí `na.rm = TRUE`**
:::

::: {#e3f3b699 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#567facd7 .cell .markdown}
**Totéž lze udělat pomocí knihovny `dplyr`**
:::

::: {#58ef91bb .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#403cc914 .cell .markdown}
**A finálně pro každou skupinu zvlášť skrze `group_by`**
:::

::: {#3f812a25 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#4ba90a1e .cell .markdown}
**Můžeme si třeba vytvořit funkci, co nám takovýto přehled vytvoří pro
náš zadaný datový rámec a příslušné třízení + data.**
:::

::: {#2b1d58fa .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#b681ad40 .cell .markdown}
**Může se hodit si takovou tabulku uložit do excelu pomocí
`openxlsx::write.xlsx()`:**
:::

::: {#0bf41cb5 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
-   **minimální a maximální hodnoty bez odlehlých pozorování**
    (čárky/whiskers),
-   **odlehlá pozorování** (body mimo čáry).

**Vykreslujeme vždy pro originální data, můžeme doplnit i vykreslení pro
data bez OP v případech, kdy byl originální graf příliš roztažený
extrémními OP.**
:::

::: {#e6298f35 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#74b54fdf .cell .markdown}
**Samozřejmě můžeme uložit stejným způsobem jako u předchozího
sloupcového grafu.**
:::

::: {#3a63c709 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#1ae628aa .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#9f71f83c .cell .markdown}
### c) Vnitřní hradby a identifikace odlehlých pozorování

-   odlehlá pozorování, které jsme viděli i v boxplotu jsou
    identifikována pomocí vnitřních hradeb
-   tato pozorování často umí znehodnotit číselné charakteristiky, proto
    je vhodné se s nimi nějakým způsobem vypořádat
-   jedním z \"universálních\" způsobů je jejich odstranění

Identifikaci odlehlých pozorování budeme provádět pomocí `dplyr` a
funkce `rstatix::identify_outliers()`

-   je třeba aby data byla ve standardním datovém formátu a obsahovala
    sloupec s identifikátorem entity (např. `id`)
-   odstranění děláme vždy pro každou skupinu zvlášť!

Následné odstranění provedeme pomocí definice nového sloupce, který bude
obsahovat pouze data bez odlehlých pozorování. **Tímto způsobem si
zachováme původní data!**
...
::: {#a3da2c3d .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#c110280f .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#61c740fd .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#d787db9c .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#f536d21f .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
boxplot(dataS$tok5 ~ dataS$vyrobce)
boxplot(dataS$tok5OP ~ dataS$vyrobce)
```
:::

::: {#0c6af90b .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#8b5187fc .cell .markdown}
**Analytik může vždy říct, že odlehlá pozorování odstraňovat nebude, ale
tuto informaci musí do zápisu o analýze uvést!**
:::

::: {#d866a8cc .cell .markdown}
### d) Histogramy a aproximace hustoty pravděpodobnosti

-   **`hist()`** -- vytvoří histogram kvantitativní proměnné, **Pozor,
    vyžaduje vstup v podobě numerického vektoru!**
    -   není ochotno fungovat pro `tibble`/`data.frame` i pokud máme jen
        jeden sloupec, je třeba jej dostat přes `data$sloupec`
    -   parametr `breaks` určuje počet intervalů
    -   parametr `freq` určuje, zda chceme zobrazit absolutní četnosti
        nebo odhad hustoty pravděpodobnosti
-   **`density()`** -- vytvoří aproximaci hustoty pravděpodobnosti z
    histogramu
...
::: {#c8314d42 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#18978f57 .cell .markdown}
Lze si hrát s parametry, přidat do jednoho grafu i odhad hustoty
pravděpodobnosti, a fit pomocí normálního rozdělení.
:::

::: {#40d613ef .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#7b794758 .cell .markdown}
A samozřejmě exportovat například do PDF.
:::

::: {#e80e8520 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#aac3e85b .cell .markdown}
### e) QQ-graf

Vykreslíme pomocí `qqnorm()` a `qqline()`. Stajně jako u histogramu, je
třeba mít data ve formátu numerického vektoru.

**Vykreslujeme vždy pro data bez odlehlých pozorování!!**
:::

::: {#c7a7decd .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#52bc84f4 .cell .markdown}

------------------------------------------------------------------------
:::

::: {#6ef1e4f3 .cell .markdown}
# 8. pravidlo 3 $\sigma$ a Čebyševova nerovnost {#8-pravidlo-3-sigma-a-čebyševova-nerovnost}

### a) Empirické ověření normality

-   provádíme vizuálně:
    -   pomocí QQ-grafu: pokud body leží na přímce, můžeme předpokládat
        normální rozdělení
...
::: {#e3dec91b-7553-49ef-a4f5-1ca00b399504 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#6063b922 .cell .markdown}
**Všechny nástroje pro ověření normality jsme si již ukázali, můžeme je
zabalit do jedné funkce:**
:::

::: {#81e28411 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#e1f9beac .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#9423af5b .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#88a74a9b .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#5cbaf393 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#03de0d51 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#007a8d67 .cell .markdown}

------------------------------------------------------------------------
:::

::: {#0c9db44b .cell .markdown}
# 9. Zaokrouhlování {#9-zaokrouhlování}

Vše potřebné k zaokrouhlování naleznete na LMS v dokumentu
[Zaokrouhlování](https://lms.vsb.cz/pluginfile.php/1298954/mod_folder/content/0/Leg%C3%A1ln%C3%AD%20tah%C3%A1ky/zaokrouhlovani.pdf).

#### To nejdůležitější:

-   Směrodatnou odchylku zaokrouhlujeme na předepsaný počet cifer nahoru
    (*ceiling*):
    -   Velikost datového souboru $\in \langle 2,10 \rangle$ → **1
        platná cifra**\
    -   Velikost datového souboru $\in (10,30 \rangle$ → **2 platné
        cifry**\
    -   Velikost datového souboru $\in (30,2000 \rangle$ → **3 platné
        cifry**
-   Míry polohy (průměry, kvantily, \...) pak zaokrouhlujeme klasicky
    (*round*) na stejnou platnou cifru jako směrodatnou odchylku.
:::

::: {#188f10cb .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#6fbb4f86 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#ad2777f3 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
...
::: {#d87afae2 .cell .code vscode="{\"languageId\":\"r\"}"}
```{R}
